    ...
    for root, root_dir_names, root_file_names in os.walk(input_dir):
        ...
        if len(root_file_names) > 0:
            ...
            output_file_path = os.path.join(
                output_work_dir, output_file_dir + ".wav")
            ...
            for index in range(data_len-1):
                ...
                if '-->' in line:
                    ...
                    start_time = line[0].strip().replace(',', '.')
                    end_time = line[-1].strip().replace(',', '.')
                    ...
                    text_txt.write(srt_file+' '+file_id+' '+lines[index+1].strip()+'\n')
                    output_num += 1
            print "已处理完 %s 下MP4文件" % (root)
            print("生成最后一个文件为: %s " % (file_id))

    print "已将所有MP4文件转换成wav文件"
    print("总时长: %s 秒" % (output_time))
