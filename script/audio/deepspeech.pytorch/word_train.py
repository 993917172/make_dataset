...
parser.add_argument('--labels_path', default='word_labels.json', help='Contains all characters for prediction')
parser.add_argument('--window_size', default=.02, type=float, help='Window size for spectrogram in seconds')
parser.add_argument('--window_stride', default=.01, type=float, help='Window stride for spectrogram in seconds')
parser.add_argument('--window', default='hamming', help='Window type for spectrogram generation')
parser.add_argument('--hidden_size', default=700, type=int, help='Hidden size of RNNs')
parser.add_argument('--hidden_layers', default=6, type=int, help='Number of RNN layers')
...
parser.add_argument('--learning_anneal', default=1.01, type=float, help='Annealing applied to learning rate every epoch')
...
parser.add_argument('--save_folder', default='models_word/', help='Location to save epoch models')
parser.add_argument('--model_path', default='models_word/deepspeech_final.pth.tar',
...
parser.add_argument('--log_dir', default='visualize_word/', help='Location of tensorboard log')
...
parser.set_defaults(cuda=True, silent=False, checkpoint=False, visdom=False, augment=False, tensorboard=True,
...
def main():
    ...
    loss_results, valid_results, cer_results, wer_results, lr_results = torch.Tensor(args.epochs), torch.Tensor(args.epochs), torch.Tensor(args.epochs), torch.Tensor(args.epochs), torch.Tensor(args.epochs)
    best_cer = None
    if args.visdom:
    ...
    if args.continue_from:
        ...
        loss_results, valid_results, cer_results, wer_results, lr_results = package['loss_results'], package['valid_result'], package[
        ...
        if args.visdom and \
            ...
            y_axis = [loss_results[0:start_epoch], valid_results[0:start_epoch], wer_results[0:start_epoch], cer_results[0:start_epoch], lr_results[0:epoch]]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter):
            ...
            if args.checkpoint_per_batch > 0 and i > 0 and (i + 1) % args.checkpoint_per_batch == 0:
                ...
                torch.save(DeepSpeech.serialize(model, optimizer=optimizer, epoch=epoch, iteration=i,
                                                loss_results=loss_results,valid_results=valid_results,
                                                wer_results=wer_results, cer_results=cer_results, lr_results=lr_results, avg_loss=avg_loss),
                           file_path)
            del loss
            del out
        ...
        print('Validation Summary Epoch: [{0}]\t'
            ...
            epoch + 1, valid_loss=valid_loss, wer=wer, cer=cer, learning_rate=optim_state['param_groups'][0]['lr']))
        ...
        if args.checkpoint:
            ...
            torch.save(DeepSpeech.serialize(model, optimizer=optimizer, epoch=epoch, loss_results=loss_results,valid_results=valid_results,
                                            wer_results=wer_results, cer_results=cer_results, lr_results=lr_results),
                       file_path)
        # anneal lr
        optim_state = optimizer.state_dict()
